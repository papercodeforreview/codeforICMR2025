...
class EarlyTrain(CoresetMethod):
    ...
    def run(self):
        ...
        if self.args.model == 'QResNet18' and self.args.selection != 'kCenterGreedy' and self.args.selection != 'Herding':
            self.model = nets.__dict__[self.args.model if self.specific_model is None else self.specific_model](
                self.args.bitwidth, self.args.channel, self.dst_pretrain_dict["num_classes"] if self.if_dst_pretrain else self.num_classes,
            ...
            # print(self.specific_model)
            # self.model = nets.__dict__[self.args.model if self.specific_model is None else self.specific_model](
            #     self.args.channel, self.dst_pretrain_dict["num_classes"] if self.if_dst_pretrain else self.num_classes,
            #     pretrained=self.torchvision_pretrain,
            #     im_size=(224, 224) if self.torchvision_pretrain else self.args.im_size).to(self.args.device)
            self.model = nets.__dict__[self.args.model if self.specific_model is None else self.specific_model]().to(self.args.device)  
